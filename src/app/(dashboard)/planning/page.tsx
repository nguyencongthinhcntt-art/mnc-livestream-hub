...
import React from 'react';
import { DollarSign, Package, Plus, Calculator, CheckCircle2, Tag, AlertTriangle, Check, TrendingUp, Wallet, ShoppingBag, Radio } from 'lucide-react';
import { prisma } from '@/lib/prisma';
import LivestreamSelector from './LivestreamSelector';
import { 
  addBudgetItem, 
...
export default async function PlanningPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      ...
      {/* 2. BANNER NỔI BẬT: BƯỚC CHỌN PHIÊN LIVE & PHÊ DUYỆT */}
      <div className="rounded-2xl border-2 border-indigo-500/40 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-5 shadow-xl text-white">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          
          {/* Hướng dẫn thao tác */}
          <div className="flex items-center gap-3.5">
            <div className="p-3 bg-indigo-600 rounded-xl text-white shadow-md shadow-indigo-500/30 flex items-center justify-center shrink-0">
              <Radio size={22} className="animate-pulse" />
            </div>
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-base font-bold text-white">Chọn Phiên Livestream Cần Thao Tác</h2>
              </div>
              <p className="text-xs text-slate-300 mt-0.5">
                Vui lòng chọn một phiên livestream cụ thể bên dưới để cấu hình ngân sách, SKU và xem P&L Simulator.
              </p>
            </div>
          </div>

          {/* Bộ Chọn Phiên & Nút Duyệt Kế Hoạch */}
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            {livestreams.length > 0 && (
              <div className="flex-1 md:flex-none min-w-[280px]">
                <LivestreamSelector livestreams={livestreams} selectedId={selectedId} />
              </div>
            )}
            ...
            {planningData && (
              <form action={async () => {
                ...
                <button
                  type="submit"
                  className={`px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all cursor-pointer shadow-md whitespace-nowrap ${
                    isApproved 
