...
export default async function PlanningPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="p-6 space-y-6 bg-slate-50 min-h-screen">
      ...
      {/* 2. BANNER NỔI BẬT: BƯỚC CHỌN PHIÊN LIVE & PHÊ DUYỆT (GIAO DIỆN NỀN TRẮNG DỒNG BỘ) */}
<div className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-sm">
  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
    
    {/* Hướng dẫn thao tác */}
    <div className="flex items-center gap-3.5">
      <div className="p-3 bg-indigo-50 text-indigo-600 rounded-xl border border-indigo-100 flex items-center justify-center shrink-0">
        <Radio size={22} className="animate-pulse" />
      </div>
      <div>
        <div className="flex items-center gap-2">
          <h2 className="text-base font-bold text-slate-900">Chọn Phiên Livestream Cần Thao Tác</h2>
        </div>
    ...
    <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
      ...
      {planningData && (
        <form action={async () => {
          ...
          <button
            type="submit"
            className={`px-4 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all cursor-pointer shadow-sm whitespace-nowrap ${
              isApproved 
                ? 'bg-slate-100 text-slate-700 hover:bg-slate-200 border border-slate-300' 
